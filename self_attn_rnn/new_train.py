...
def sample(data_train, data_val, data_test, lexsub_nmt, label_data, all_para, sample_rate, args):
    # DLK add paraphrases
    ...
    if lexsub_nmt:
        for dataset in [data_val, data_test]:
            for item in data_val:
                try:
                    sent = ' '.join(json.loads(item)['text'])
                except:
                    print("error loating json")
                    print("info = json.loads(sent)")
                    pdb.set_trace()
                test_set.add(sent)
        ...
        if all_para is not None and random.random() < sample_rate and item_label in all_para and len(all_para[item_label]) > 0:
            ...
            if alternative:
                if not lexsub_nmt:
                    ...
                    sampled += 1
                elif lexsub_nmt and alternative not in test_set:
                    splitalt = alternative.split()
                    textalt = str(splitalt).replace(" '", ' "').replace("',", '",').replace("['", '["').replace("']",                                                                                                '"]')
                    newline = '{"label": ' + str(item_label_int) + ',"text": ' + textalt + '}\n'
                    # check the quality of the string
                    ...
                    sampled += 1
                elif lexsub_nmt and alternative in test_set:
                    pruned += 1
                    continue
        else:
            ...
            if label_count[label] < REALMAX:
                if all_para is not None and label in all_para and len(all_para[label]) > 0:
                    ...
                    if type(alts) == list:
                        ...
                        pdb.set_trace()
    elif 'simple' in args.sampling_schema:
        for label in label_count:
            if all_para is not None and label in all_para and len(all_para[label]) > 0:
                quantity = int(label_count[label] * sample_rate)
                paras = all_para[label]
                candidates = [x[0] for x in paras]
                scores = [x[1] for x in paras]
                label_int = string_to_label[label]
                alts = random.choices(candidates, weights=scores, k=quantity)
                if type(alts) == list:
                    for alt in alts:
                        splitalt = alt.split()
                        textalt = str(splitalt).replace(" '", ' "').replace("',", '",').replace("['", '["').replace("']", '"]')
    ...
    print("Pruned {} samples to training data".format(pruned))
...
#    I = I.to(device)
    ...
    for epoch in range(args.epochs):
        ...
        data_train, uniform_train = sample(pre_para_data_train, data_val, data_test, lexsub_nmt, label_data, data_paras, new_sample_rate, args)
        if 'uniform' in args.sampling_schema:
